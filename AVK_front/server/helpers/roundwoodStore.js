 ...
 * @typedef {{ id: number, sideAMm: number, sideBMm: number, lengthMm: number, qty: number, createdAt: string }} BrusStockItem
 */

/**
 * @typedef {{
 *   id: string,
 *   kind: 'received' | 'brus_received' | 'receive_cancelled' | 'band_consumed' | 'stock_updated' | 'stock_cleared',
 *   at: string,
...
/**
 * @typedef {{ stock: RoundwoodStockItem[], brusStock: BrusStockItem[], journal: RoundwoodJournalEntry[] }} RoundwoodState
 */

const DEFAULT_STATE = /** @type {RoundwoodState} */ ({ stock: [], brusStock: [], journal: [] })
...
export async function readRoundwood() {
  ...
  const brusStock = Array.isArray(raw.brusStock) ? raw.brusStock : []
  const journal = Array.isArray(raw.journal) ? raw.journal : []
  return { stock, brusStock, journal }
}
...
export async function writeRoundwood(state) {
  const journal =
    state.journal.length > MAX_JOURNAL
      ...
      : state.journal
  await writeJson(ROUNDWOOD_FILE, { stock: state.stock, brusStock: state.brusStock ?? [], journal })
}
