...
import type { TaskDimensionRow, TaskKind, TaskPalletTarget, TaskStationAssignments, WorkTask } from '../types/task'
...
export type BandCutLine = { thicknessMm: number; stripQty?: number; doneQty?: number }
...
export async function recordCircularSawCut(
  taskId: string,
  payload: {
    thicknessMm: number
    widthMm: number
  ...
  const res = await fetch(apiUrl(`/api/tasks/${encodeURIComponent(taskId)}/circular-saw/cut`), {
    method: 'POST',
    headers: headersJson(),
    body: JSON.stringify(payload),
  })
  const data = (await res.json().catch(() => ({}))) as { task?: WorkTask; error?: string }
  if (!res.ok) {
    throw new Error(data.error ?? 'Не вдалося записати розкрій')
  }
  if (!data.task) throw new Error('Некоректна відповідь сервера')
  notifyWorkTasksChanged()
  return data.task
...
export async function buildPallets(
  ...
  const res = await fetch(apiUrl(`/api/tasks/${encodeURIComponent(taskId)}/pallets/build`), {
    method: 'POST',
    headers: headersJson(),
    body: JSON.stringify(payload),
  })
  const data = (await res.json().catch(() => ({}))) as { task?: WorkTask; error?: string }
  if (!res.ok) {
    throw new Error(data.error ?? 'Не вдалося створити піддон')
  }
  if (!data.task) throw new Error('Некоректна відповідь сервера')
  notifyWorkTasksChanged()
  return data.task
