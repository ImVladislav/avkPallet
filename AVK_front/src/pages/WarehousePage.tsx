...
export function WarehousePage() {
  ...
  const [receivedBrusStock, setReceivedBrusStock] = useState<BrusStockItem[]>([])
  ...
  const reloadRoundwood = useCallback(() => {
    ...
    void (async () => {
      try {
        const { stock, brusStock } = await fetchRoundwoodState()
        if (rwSeq.current !== n) return
        setRoundwoodCount(stock.length)
        setReceivedBrusStock(brusStock)
        setRoundwoodErr(null)
      } catch (e) {
        if (rwSeq.current !== n) return
        setRoundwoodCount(null)
        setReceivedBrusStock([])
  ...
  const receivedBrusTotals = useMemo(
    () => receivedBrusStock.reduce((sum, item) => sum + item.qty, 0),
    [receivedBrusStock],
  )

  const palletRows = useMemo(() => {
    const byType = new Map<string, { name: string; qty: number }>()
    for (const task of tasks) {
      for (const build of task.palletAssembly?.builds ?? []) {
        const key = build.palletTypeId
        const prev = byType.get(key)
        byType.set(key, {
          name: build.palletTypeName,
          qty: (prev?.qty ?? 0) + build.qty,
        })
      }
    }
  ...
  return (
    <section className="panel">
      ...
      <p className="warehouseLead">
        Залишки за завданнями на сервері. Прийом деревини — окрема сторінка; повне очищення кругляка —
        тут (бригадир / адмін).
      </p>
      ...
      <div className="warehouseGrid">
        <div className="warehouseCard">
          <div className="warehouseCardHead">
            ...
            <span className="warehouseCardMeta" title="Прийом — «Прийом деревини»">
              Прийом окремо
            </span>
          </div>
        ...
        <div className="warehouseCard">
          <div className="warehouseCardHead">
            ...
            <span className="warehouseCardMeta">Прийнятий + план циркулярки</span>
          </div>
          <div className="warehouseCardBody">
            {receivedBrusStock.length > 0 && (
              <>
                <table className="warehouseTable">
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    {receivedBrusStock.map((item) => (
                      <tr key={item.id}>
                        <td className="warehouseNum">{fmtMm(item.sideAMm)}</td>
                        <td className="warehouseNum">{fmtMm(item.sideBMm)}</td>
                        <td className="warehouseNum">{fmtMm(item.lengthMm)}</td>
                        <td className="warehouseNum">{item.qty}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                <div className="warehouseFoot">
                  Прийнятий брус: <span className="warehouseNum">{receivedBrusTotals}</span> шт.
                </div>
              </>
            )}
            {brusRows.length === 0 ? (
        ...
        <div className="warehouseCard">
          <div className="warehouseCardHead">
            <h2>
              Піддони
              <span className="warehousePill">готові</span>
            </h2>
          </div>
          <div className="warehouseCardBody">
            {palletRows.length === 0 ? (
              <p className="warehouseEmpty" style={{ margin: 0 }}>
                Готових піддонів ще немає.
              </p>
            ) : (
              <table className="warehouseTable">
                <thead>
                ...
                <tbody>
                  {palletRows.map((row) => (
                    <tr key={row.name}>
                      <td>{row.name}</td>
                      <td className="warehouseNum">{row.qty}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
