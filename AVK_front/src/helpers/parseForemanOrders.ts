...
export function orderLinesFromDimensionRows(
  ...
  unitDefault: 'mm' | 'cm',
  options?: { requireBoardLength?: boolean },
): { ok: true; lines: OrderLine[] } | { ok: false; error: string } {
  const requireBoardLength = options?.requireBoardLength ?? true
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    const lengthMm = requireBoardLength
...
export function formatOrderLinesAsText(lines: OrderLine[]): string {
  return lines
    .map((l) => {
      const base = `${l.qty} ${Math.round(l.aMm)} ${Math.round(l.bMm)}`
